...
1. Set `SANJIV_REPLAY_SPEED=1`, run `npm run sanjiv -- services` and `npm run db:upgrade`, then start `npm run dev:api` and `npm run dev:web` close together so the 70-second fixture movement is visible.
