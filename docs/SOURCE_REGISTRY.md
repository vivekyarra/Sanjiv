...
| Phase 8 energy validation catalogue | replay | Historical-replay workflow and cross-disruption validation | none | `data/replay/energy-validation-v1` contains 21 checksummed CC0 `SYNTHETIC_FIXTURE` cases generated by `Sanjiv deterministic replay generator v1`. It is legally redistributable synthetic validation data, never live data or recorded history. |
| Phase 8 LPG network | fixture | Commodity-specific LPG routes, suppliers, terminals, compatibility, flows, plans, and exports | none | `data/fixtures/lpg/india-lpg-network-v1.json` is a checksummed CC0 `SYNTHETIC_FIXTURE` in `tonne_per_day`. Capacity, demand, availability, and costs are expiring assumptions; reserve policy is `NOT_APPLICABLE`; no cargo or commercial availability is claimed. |
